...
const FLOWDESK_APP_VERSION = '20.3.45'
...
function buildSystemMark(value) {
  const next = normalizeSystemName(value)
...
function FlowDeskShell({ authSession, onLogout }) {
  ...
  const [systemName, setSystemName] = useState(() => {
    ...
    return normalizeSystemName(window.localStorage.getItem('flowdesk-system-name') || DEFAULT_SYSTEM_NAME)
  })
  ...
  return (
    <div className={`product-shell ${sidebarOpen ? 'sidebar-open' : ''} ${active === 'board' ? 'has-context' : ''}`}>
      ...
      <main className="main-canvas">
        ...
        {active === 'settings' && <SettingsPage systemName={systemName} setSystemName={setSystemName} displaySystemName={displaySystemName} themeOptions={themeOptions} uiTheme={uiTheme} setUiTheme={setUiTheme} appearanceMode={appearanceMode} setAppearanceMode={setAppearanceMode} motionLevel={motionLevel} setMotionLevel={setMotionLevel} customTheme={customTheme} setCustomTheme={setCustomTheme} iconStyleMode={iconStyleMode} setIconStyleMode={setIconStyleMode} resolvedIconStyle={resolvedIconStyle} modules={modules} collections={visibleCollections} setCollections={setCollections} moduleIcons={moduleIcons} setModuleIcons={setModuleIcons} baseTableIcons={baseTableIcons} setBaseTableIcons={setBaseTableIcons} setReminders={setReminders} />}
      </main>
...
function SettingsPage({ systemName, setSystemName, displaySystemName, themeOptions, uiTheme, setUiTheme, appearanceMode, setAppearanceMode, motionLevel, setMotionLevel, customTheme, setCustomTheme, iconStyleMode, setIconStyleMode, resolvedIconStyle, modules, collections, setCollections, moduleIcons, setModuleIcons, baseTableIcons, setBaseTableIcons, setReminders }) {
    ...
    <div className="settings-layout settings-hub-layout">
      ...
      {settingsView === 'home' && (
        ...
        <section className="panel wide settings-panel fd30-appearance-panel fd31-vivid-appearance-panel">
          ...
          <p className="settings-note">切換後會立即套用到主要按鈕、標籤、分頁、進度條、卡片重點色、輸入框 focus 色與甘特圖任務條。v20.3.45 加入外觀設定快速導覽、動效安全提醒與手機版收斂補強，外觀功能更多但操作更不亂。</p>
      ...
      {settingsView === 'system' && (
        <section className="panel settings-panel settings-detail-panel">
          ...
          <div className="fd45-system-name-card">
            <div>
              <span>系統名稱</span>
              <strong>{displaySystemName}</strong>
              <small>調整左上角顯示名稱；此設定只存在本機，不影響登入與資料庫。</small>
            </div>
            <div className="fd45-system-name-controls">
              <input
                value={systemName}
          ...
          <div className="settings-info-list">
            <div><span>目前系統名稱</span><strong>{displaySystemName}</strong></div>
