...
const FLOWDESK_APP_VERSION = '20.4.12'
        ...
        <div className="purchase-home-list">
          {purchases.length ? purchases.slice(0, 5).map((row) => (
            <button key={row.id} type="button" onClick={() => setActive('base')}>
              <div><strong>{purchaseCardTitle(row)}</strong><small>{row.department || '未指定單位'} · {row.vendor || '未指定廠商'} · {getPurchaseItems(row).length} 項</small></div>
...
function ProjectManagementPage({ projects: initialProjectRows = [], onCreateWorkItem }) {
  ...
  function applyProjectTaskDatePatchDirect(projectId, taskIndex, subtaskIndex, patch = {}, recordText = '') {
    if (!projectId) return
    setProjects((rows) => rows.map((project) => {
      if (project.id !== projectId) return project
      const safeProject = normalizeProject(project)
      const tasks = (safeProject.tasks || []).map((task, index) => {
        ...
        if (subtaskIndex === null || subtaskIndex === undefined) {
          return normalizeTask({ ...task, ...patch }, safeProject, index)
        }
        const subtasks = (task.subtasks || []).map((subtask, currentSubIndex) => (
          currentSubIndex === subtaskIndex ? { ...subtask, ...patch } : subtask
        ))
        return normalizeTask({ ...task, subtasks }, safeProject, index)
      })
      const nextProject = normalizeProject({ ...safeProject, tasks })
      nextProject.progress = estimateProjectProgress(nextProject)
      if (recordText) {
        nextProject.records = [`${new Date().toLocaleString('zh-TW', { hour12: false })}｜${recordText}`, ...(safeProject.records || [])].slice(0, 30)
      }
      return nextProject
    }))
  }
  ...
  function shiftProjectTaskDates(projectId, taskIndex, deltaDays) {
    ...
    applyProjectTaskDatePatchDirect(projectId, taskIndex, null, { start: nextStart, end: nextEnd, subtasks: shiftedSubtasks }, `整段平移任務「${task.name || '未命名任務'}」${deltaDays > 0 ? '往後' : '往前'} ${Math.abs(deltaDays)} 天。`)
  }
  ...
  function shiftProjectSubtaskDates(projectId, taskIndex, subtaskIndex, deltaDays) {
    ...
    const nextStart = addDaysToDateValue(currentStart, deltaDays)
    const nextEnd = addDaysToDateValue(currentEnd, deltaDays)
    applyProjectTaskDatePatchDirect(projectId, taskIndex, subtaskIndex, { start: nextStart, end: nextEnd }, `整段平移子任務「${subtask.name || '未命名子任務'}」${deltaDays > 0 ? '往後' : '往前'} ${Math.abs(deltaDays)} 天。`)
  }
  ...
  function startGanttDateDrag(project, scope, taskIndex, edge, event, subtaskIndex = null) {
    ...
    const applyMove = (moveEvent) => {
      ...
      if (scope === 'task' && originalTask) {
        if (edge === 'move') {
          ...
          applyProjectTaskDatePatchDirect(safeProject.id, taskIndex, null, { start: nextStart, end: nextEnd, subtasks: shiftedSubtasks })
        } else if (edge === 'start') {
          ...
          applyProjectTaskDatePatchDirect(safeProject.id, taskIndex, null, { start: nextStart })
        } else {
          ...
          applyProjectTaskDatePatchDirect(safeProject.id, taskIndex, null, { end: nextEnd })
        }
        return
      }
      ...
      if (scope === 'subtask' && originalSubtask) {
        if (edge === 'move') {
          ...
          applyProjectTaskDatePatchDirect(safeProject.id, taskIndex, subtaskIndex, { start: nextStart, end: nextEnd })
        } else if (edge === 'start') {
          ...
          applyProjectTaskDatePatchDirect(safeProject.id, taskIndex, subtaskIndex, { start: nextStart })
        } else {
          ...
          applyProjectTaskDatePatchDirect(safeProject.id, taskIndex, subtaskIndex, { end: nextEnd })
        }
      }
    }
    ...
    const stopMove = () => {
      ...
      updateProject(safeProject.id, {}, `使用甘特圖${actionText}${scopeText}期程。`)
    }
  ...
  function renderGantt(project, options = {}) {
    ...
    return (
      <div className={`fd203-gantt-panel fd203-gantt-fit-${fitMode}${embedded ? ' embedded' : ''}${compact ? ' compact' : ''}`} data-week-count={weekCount} data-fit-mode={fitMode}>
        <div className="fd203-gantt-summary fd20411-sticky-summary">
          <div>
        ...
        <div className="fd203-gantt-scroll">
          <div className="fd203-gantt-grid fd203-gantt-head fd20411-sticky-head" style={{ gridTemplateColumns: gridColumns }}>
            <span>項目</span>
            {weekTicks.map((tick) => (
          ...
          <div className="fd203-gantt-grid fd203-gantt-row fd20411-sticky-project-row" style={{ gridTemplateColumns: gridColumns }}>
                ...
                <div className={`fd203-gantt-grid fd203-gantt-row task ${subtaskCount ? 'has-subtasks' : 'no-subtasks'} ${subtasksOpen ? 'subtasks-open' : 'subtasks-collapsed'} ${task.done ? 'is-complete' : 'is-incomplete'}`} style={{ gridTemplateColumns: gridColumns }}>
                  <div className="fd203-gantt-label" title={dateRangeLabel(taskStart, taskEnd)}>
                    ...
                    <div className="fd203-gantt-row-actions compact-v16 fd203-gantt-row-actions-v29">
                      ...
                      <button type="button" className="fd203-mini-link fd20406-shift fd20411-shift-button" title="任務整段往前 1 天" onPointerDown={(event) => event.stopPropagation()} onMouseDown={(event) => event.stopPropagation()} onClick={(event) => { event.preventDefault(); event.stopPropagation(); shiftProjectTaskDates(project.id, index, -1) }}>←1日</button>
                      <button type="button" className="fd203-mini-link fd20406-shift fd20411-shift-button" title="任務整段往後 1 天" onPointerDown={(event) => event.stopPropagation()} onMouseDown={(event) => event.stopPropagation()} onClick={(event) => { event.preventDefault(); event.stopPropagation(); shiftProjectTaskDates(project.id, index, 1) }}>1日→</button>
                      <button type="button" className="fd203-mini-link fd20406-shift week fd20411-shift-button" title="任務整段往前 1 週" onPointerDown={(event) => event.stopPropagation()} onMouseDown={(event) => event.stopPropagation()} onClick={(event) => { event.preventDefault(); event.stopPropagation(); shiftProjectTaskDates(project.id, index, -7) }}>←1週</button>
                      <button type="button" className="fd203-mini-link fd20406-shift week fd20411-shift-button" title="任務整段往後 1 週" onPointerDown={(event) => event.stopPropagation()} onMouseDown={(event) => event.stopPropagation()} onClick={(event) => { event.preventDefault(); event.stopPropagation(); shiftProjectTaskDates(project.id, index, 7) }}>1週→</button>
                    ...
                    <div className={`fd203-gantt-grid fd203-gantt-row subtask ${subtask.done ? 'is-complete' : 'is-incomplete'}`} key={subtaskKey} style={{ gridTemplateColumns: gridColumns }}>
                      <div className="fd203-gantt-label subtask" title={dateRangeLabel(subStart, subEnd)}>
                        ...
                        <div className="fd203-gantt-row-actions compact-v16">
                          ...
                          <button type="button" className="fd203-mini-link fd20406-shift fd20411-shift-button" title="子任務整段往前 1 天" onPointerDown={(event) => event.stopPropagation()} onMouseDown={(event) => event.stopPropagation()} onClick={(event) => { event.preventDefault(); event.stopPropagation(); shiftProjectSubtaskDates(project.id, index, subIndex, -1) }}>←1日</button>
                          <button type="button" className="fd203-mini-link fd20406-shift fd20411-shift-button" title="子任務整段往後 1 天" onPointerDown={(event) => event.stopPropagation()} onMouseDown={(event) => event.stopPropagation()} onClick={(event) => { event.preventDefault(); event.stopPropagation(); shiftProjectSubtaskDates(project.id, index, subIndex, 1) }}>1日→</button>
